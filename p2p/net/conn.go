...
import (
	...
	"github.com/smallnest/libp2p/p2p/net/wire"
...
func newConnection(conn readWriteCloseAddresser, netw networker, formatter wire.Formatter, remotePub crypto.PublicKey) *FormattedConnection {
...
func (c *FormattedConnection) shutdown(err error) {
	log.Info("(%v) shutdown. id=%s err=%v", c.remotePub.String(), c.id, err)
	c.formatter.Close()
